...
class YouTubeSkill(Skill):
    ...
    # **В каталог для модели этот инструмент не идёт.** «Включи Break My Heart»
    # модель выбирала здесь, и ассистент уводил владельца с открытой Яндекс
    # Музыки на ютуб — притом послушно, в его же вкладке. Правило владельца
    # простое: раз я на музыкальном сайте, значит речь про музыку; раз на
    # ютубе — про видео. Ровно это и делает `page.play_item`: он ищет на том
    # сайте, который открыт. Поэтому неоднозначное «включи X» достаётся ему, а
    # здесь остаются фразы, где про видео сказано прямо.
    @tool(routable=False,
          phrases=["включи видео {query}", "поставь {query} на ютубе",
