...
class _Extension:
    ...
    def __init__(
        ...
        *,
        ...
        #: Ждали ли уже: второй раз ждать нечего, ответ известен.
        self._waited = False
    ...
    async def ready(self, timeout: float = 0.0) -> bool:
        ...
        Служебный поток браузера просыпается по будильнику, а чаще раза в
        полминуты Chrome его не пускает — поэтому после запуска Jarvis браузера
        может не быть на связи ещё десяток секунд. Живой случай: `--say "включи
        трек …"` отработал за 0.18 с, получил «расширение не подключено» и увёл
        просьбу на ютуб, а расширение поздоровалось через девять секунд.

        **Ждём один раз за весь запуск.** Иначе на машине, где расширение не
        поставлено вовсе, каждая команда к странице тормозила бы на этот срок —
        а ответ известен заранее. И не ждём, если оно уже приходило: обрыв
        посреди работы дело обычное, вешать на нём команду незачем.
        """
        ...
        try:
            ...
            self._log.info("Расширение так и не подключилось — работаю без него")
            return False
        return self._server.connected
