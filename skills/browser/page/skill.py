...
#: Сколько минут факт о происходящем остаётся правдой (см. `core/situation.py`).
#: Сайт держится дольше трека: вкладки живут часами, а трек — минуты. Числа
#: тут не про точность, а про то, когда подсказка становится враньём: модель
#: поверит и уведёт команду туда, где давно ничего не открыто.
SITE_FRESH_MIN = 20.0
TRACK_FRESH_MIN = 5.0

#: Слова, после которых «включи X» — про видео, а не про трек: такое уходит на
#: `video_site`, а не в Яндекс Музыку.
VIDEO_WORDS = frozenset({"ролик", "ролики", "клип", "клипы", "видео", "видос", "видосик", "трейлер"})
...
#: По этим словам в отказе видно, что расширения нет вовсе: браузер закрыт или
#: скилл `browser` не подключён. От «страница не отвечает» отличается тем, что
#: сюда приходят обычным ответом, а не исключением, — и раньше такой отказ
#: просто зачитывался вслух, хотя музыку рядом играл AIMP (24.09.2026, 18:07).
NO_EXTENSION_MARKS = ("не подключено", "не подключён", "не подключилось")


def no_extension(error: object) -> bool:
    """Отказ из-за отсутствующего расширения, а не из-за самой страницы."""
    said = str(error or "").lower()
    return any(mark in said for mark in NO_EXTENSION_MARKS)
...
ACTIONS: dict[str, tuple[dict[str, Any], ...]] = {
    ...
    "search": (
        {"label": ["поиск", "найти", "search", "искать"]},
        {"click": ['[type="search"]', '[role="searchbox"]']},
    ),
...
#: Сайты, на которых Jarvis умеет искать сам: у них есть своя строка поиска.
#:
#: Список нужен для одного решения — стоит ли вообще смотреть на страницу.
#: «Включи трек X» с открытой веб-панелью не должно ничего искать в её тексте:
#: там нет ни треков, ни поиска по ним. Домен сравнивается с хвоста, поэтому
#: запись работает и для поддоменов.
SEARCHABLE: tuple[str, ...] = ("music.yandex.ru", "youtube.com", "vk.com")

#: Сколько раз перепроверить выдачу и с какой паузой.
#:
#: Приложение сайта дорисовывает результаты своим кодом, и сколько это займёт,
#: зависит от сети. Восемь попыток по 0.7 с — это около шести секунд, и все они
#: честные: каждая смотрит на живой DOM. Владелец сказал прямо, что скорость тут
#: не важна, — а вот промахнуться мимо ещё не отрисованной выдачи неприятно.
SEARCH_ATTEMPTS = 8
SEARCH_DELAY = 0.7
...
def searchable(host: str) -> bool:
    """Есть ли у сайта своя строка поиска, в которую имеет смысл печатать."""
    if not host:
        return False
    return any(host == name or host.endswith(f".{name}") for name in SEARCHABLE)
...
INTENT_TEXT: dict[str, str] = {
    ...
    "search": "открыть строку поиска на сайте",
    "mute": "выключить звук",
...
class PageSkill(Skill):
    ...
    @tool(phrases=["включи трек {track}", "включи песню {track}",
                   ...
                   # «Включи Linkin Park Faint» — так просят музыку на самом
                   # деле, без слова «трек» (живой запуск 24.09.2026: фраза не
                   # нашлась, ушла в разговор и получила ответ на английском).
                   # Фраза широкая, и держится она на том, что точные фразы и
                   # более частные шаблоны проверяются раньше: «включи оверлей»,
                   # «включи блютуз», «включи игровой режим» разбираются своими
                   # командами. Замер (tools/offline_bench.py) — 57% → 60%
                   # понятого без сети, ни одной подмены на 320 живых репликах.
                   #
    ...
    async def play_item(self, track: str, site: str = "") -> ToolResult:
        ...
        **Правило сменил владелец 14.09.2026**: «трек вообще должен был найтись в
        ...
        if first.lower().strip(" ,.") in VIDEO_WORDS and rest.strip():
            self.log.info("В просьбе сказано %r — это видео, а не трек", first)
            return await self.play_video(rest.strip(), site=site)
        if not site.strip():
    ...
    @tool(routable=False,
          phrases=["включи видео {track}", "открой видео {track}",
                   "включи ролик {track}", "открой ролик {track}", "поставь ролик {track}",
                   "включи клип {track}", "открой клип {track}", "поставь клип {track}",
                   "включи видос {track}", "открой видос {track}",
    ...
    # «прокути» — не опечатка: так Whisper слышит «прокрути», и без этой
    # строки каждая просьба пролистать уезжала в платную модель. Ровно то, о
    # чём резолвер и пишет в лог: «Повторяется — добавь её в phrases скилла».
    ...
    async def now_playing(self, site: str = "") -> ToolResult:
        ...
        # Раз уж спросили — пусть знает и модель. «Сделай потише» с этой строкой
        # в подсказке однозначно про музыку, а без неё — про что угодно.
        # Срок короткий: трек меняется, и вчерашнее название хуже, чем ничего.
        self.situation.note("играет", track, minutes=TRACK_FRESH_MIN)
    ...
    async def _act(self, action: str, **options: Any) -> ToolResult:
        ...
        Обёртка ловит два случая, и оба кончаются одинаково — плеером на
        машине, если музыку можно переключить им.

        Первый: **страница не отвечает**. Тогда остальные способы не
        перебираются — каждый упёрся бы в ту же молчащую вкладку.

        Второй: **расширения нет вовсе**, потому что браузер закрыт. Исключения
        тут не бросается, отказ приходит обычным ответом, и 24.09.2026 в 18:07
        это стоило владельцу команды: «переключи трек» при закрытом браузере
        ответило «расширение не подключено», хотя рядом играл AIMP. Отказ
        звучал как отказ работать, а не как «я не туда посмотрел».
        """
        try:
            result = await self._act_inner(action, **options)
        ...
        if result.ok or not no_extension(result.error):
            return result
        local = await self._local_music(action)
        return local if local is not None else result
    ...
    async def _act_inner(
        ...
        *,
        ...
        # Сайт мы только что узнали, спрашивая вкладку, — значит он достаётся
        # даром, и его стоит положить в обстановку. Модели это отвечает на
        # главный вопрос «включи X»: включать-то где. Срок короткий: вкладку
        # закрывают, а подсказка «открыта Яндекс Музыка» через три часа уже не
        # подсказка, а ложь, в которую модель поверит.
        if host:
            self.situation.note("открыт сайт", host, minutes=SITE_FRESH_MIN)
        ...
        # `searchable` знает, у каких сайтов есть своя строка поиска. Нет её —
        # сайт не про музыку и не про видео, и сравнивать названия на нём незачем.
        if search and otherwise is not None and not searchable(host):
            self.log.info("На %s искать нечем — на странице даже не смотрю", host or "этой странице")
            return await otherwise()

        result = await self._run(steps, tab)
        if search and result is None:
            # На странице этого нет — поищем на самом сайте, как это сделал бы
            # человек: нажать «Поиск», напечатать, включить верхнее.
            #
            # Только когда **не нашли**. «Нашли, но тишина» искать заново незачем:
            # строка на месте, и все способы включить её страница уже перебрала, —
            # а повторный поиск это ещё десяток нажатий по чужой разметке.
    ...
    async def _through_search(
        ...
        """Поискать названное **на самой странице** и включить первое найденное.

        Три шага, ровно как это делает человек: нажать «Поиск», напечатать
        название, включить верхний результат.

        **Адресом выдачи это делать нельзя, хотя так проще.** Переход по ссылке
        перезагружает приложение сайта целиком — и глушит то, что играло. Ради
        «включи следующий трек» терять текущий недопустимо, а именно это и
        происходило: вкладка уезжала на `/search?text=…`, музыка обрывалась, и
        дальше Jarvis ещё несколько секунд смотрел на недорисованную страницу.
        Работать с тем, что открыто, — просьба владельца и просто правильнее.

        Возвращаемся к нажатиям и печати: это те же шаги, которыми уже работают
        «нажми поиск» и «введи в поиск», то есть проверенный путь, а не новый.
        """
        if not searchable(host):
            return None

        # 1. Открыть строку поиска. Может быть уже открыта — тогда нажатие
        #    ничего не портит, и неудача здесь не приговор.
        opened = await self._run(self._plan("search", host), tab)
        self.log.info(
            "Ищу %r на самой странице: %s",
            query,
            "нажал поиск" if opened is not None else "поиск и так открыт",
        )
        await asyncio.sleep(SEARCH_OPEN_DELAY)

        # 2. Напечатать и отправить. Печатать некуда — дальше бессмысленно.
        typed = await self._run([{"type": query, "submit": True}], tab)
        if typed is None:
            self.log.info("Строку поиска на %s найти не удалось", host)
            return None

        # 3. Дождаться выдачи и включить верхний результат.
        #
        ...
        # совпадение по словам увело в ремиксы посторонних артистов ниже.
        #
        ...
        # Повторяем только пока **не нашли**: выдачу сайт дорисовывает своим
        # кодом, и первые попытки честно натыкаются на пустоту. А вот «нашли, но
