...
class PageSkill(Skill):
    ...
    async def _act(self, action: str, **options: Any) -> ToolResult:
        ...
        **Не вышло во вкладке — отдаём плееру на машине.** Любая неудача, а не
        какая-то особенная: 24.09.2026 этот запасной путь чинился трижды и
        дважды мимо, потому что вешался на конкретный способ отказа.

        * 17:5x — ловилось исключение «страница не отвечает». Оно бросается,
          только когда браузер **открыт**, а вкладка молчит;
        * 18:07 — браузер был закрыт совсем, и отказ пришёл обычным ответом,
          мимо исключения;
        * 20:27 — браузер открыт, вкладка ответила, но нажимать в ней нечего:
          «Не нашёл, чем это сделать на странице», — а музыка играла в AIMP.

        Общее у всех трёх одно: **во вкладке не вышло**. На это и вешаем, а
        разбирать, почему именно, незачем — человек просил переключить музыку,
        и где она играет, его не касается. Не музыкальное действие (лайк,
        перемотка) местному плееру не отдать, и там остаётся прежний отказ.
        """
        try:
