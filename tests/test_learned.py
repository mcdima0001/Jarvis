...
def test_a_template_never_starts_with_a_slot() -> None:
    """Живой случай 24.09.2026: выученное «{question} на экране» поймало
    «дважды нажми на зелёную кнопку на экране».

    Такой шаблон опознаётся одним хвостом, а голову забирает целиком — вместе
    с чужим глаголом. Ассистент спросил у зрячей модели «дважды нажми на
    зелёную кнопку», та ответила «Готово», и это прозвучало вслух: мышь не
    шевельнулась, а успех был объявлен. Первое слово — единственная опора, по
    которой видно, о чём вообще речь.
    """
    key, stored = generalize("посмотри что на экране", {"question": "посмотри что"})
    assert key == "посмотри что на экране", "шаблона из этого делать нельзя"
    assert stored == {"question": "посмотри что"}


def test_a_slot_after_the_first_word_is_still_fine() -> None:
    """Запрет узкий: опора — первое слово, а не отсутствие подстановок вообще."""
