...
class SkillMeta:
    ...
    #: Как скилл называют вслух: ``("браузер", "browser")``. Нужно там, где имя
    #: модуля приходит из речи, — «переподключи модуль браузер».
    #:
    #: Объявляет их **сам скилл**, и это не формальность. Половину имён нельзя
    #: вывести из английского названия никаким сравнением: «браузер» и
    #: `browser` — перевод, а не написание, и общего у них ровно ничего.
    #: Держать такой словарь в ядре значило бы дописывать его при каждом новом
    #: скилле — то есть править ядро ради плагина.
    spoken: tuple[str, ...] = field(default_factory=tuple)

    @property
    def names(self) -> tuple[str, ...]:
        """Все имена скилла: настоящее плюс произносимые."""
        return (self.name, *self.spoken)
