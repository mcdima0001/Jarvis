...
_DIALOG_SYSTEM = {
    "ru": (
        ...
        "вслух. Без списков и разметки. Приборами отсюда ты не управляешь и "
        "ничего не открываешь, поэтому **ни в какой форме** не утверждай, что "
        "что-то открыто, включено, найдено или сделано: ни «включаю», ни "
        "«открыт», ни «готово» — ты этого не делал. И только если тебя "
        "прямо просят выполнить действие — повелительным «сделай», «открой», "
        "«найди» — скажи, что такой команды не понял, и попроси сказать иначе. "
        "На вопрос, шутку, болтовню или простое сообщение о себе («я уже поел», "
        "«устал») так отвечать нельзя: ответь по-человечески, коротко — это и "
        "есть твоя работа здесь. Фраз «не вижу команды» и «скажите, что нужно "
        "сделать» в разговоре не бывает."
    ),
    "en": (
...
#: «Перезагрузил 21 модуль» — вслух сокращений и цифр без слова не бывает.
_MODULES = ("модуль", "модуля", "модулей")
...
class CoreTools:
    ...
    @tool(
        name="repeat",
        phrases=["попробуй ещё раз", "попробуй еще раз", "попробуй снова", "давай ещё раз",
                 "давай еще раз", "повтори команду", "сделай ещё раз", "сделай еще раз",
                 "try again", "do it again"],
        reversible=False,
    )
    async def repeat(self) -> ToolResult:
        """Повторить прошлую команду ещё раз — «попробуй ещё раз».

        Сама повторяет диспетчер: только он проводит реплику заново тем же путём,
        что и сказанную вслух. Сюда вызов доходит, лишь если его позвали мимо
        диспетчера (например, планом), — тогда честно отказываемся.

        Живой случай 14.09.2026: «попробуй ещё раз» модель дважды разобрала во
        включение трека с названием «попробуй ещё раз».
        """
        return ToolResult.failure(
            "повторить прошлую команду может только прямая просьба",
            speech={"ru": "Повторить могу только по прямой просьбе.", "en": "I can only repeat on a direct request."},
        )
    ...
    @tool(
        ...
        routable=False,
        phrases=[
            "переподключи все модули",
            "перезагрузи все модули",
            "перезапусти все модули",
            "обнови все модули",
            "переподключи модули",
            "перезагрузи модули",
            # Whisper слышит приставку как «пре-»: живой случай 24.09.2026,
            # 18:06 — «преподключи все модули». Одной буквой это ловит и
            # резолвер `similar`, но лишняя строка тут дешевле промаха.
            "преподключи все модули",
            "reload all modules",
            "reload all skills",
        ],
        reversible=False,
    )
    async def reload_all(self) -> ToolResult:
        """Перезагрузить с диска все загруженные модули разом.

        Появился 24.09.2026 по живому промаху: «переподключи все модули»
        такой команды не было, фраза ушла в модель, та выбрала «прими все
        черновики» — и ассистент бодро ответил «Черновиков нет». Ответ звучал
        как ответ, а не как отказ, и владелец решил, что модули обновлены.

        Отдельным инструментом, а не хитростью внутри `reload_skill`: у того
        аргумент — имя модуля, и «все» пришлось бы разбирать как имя, то есть
        однажды перезагрузить модуль с похожим названием.
        """
        names = list(self._skills.loaded)
        if not names:
            return ToolResult.failure(
                "загруженных модулей нет",
                speech={"ru": "Загруженных модулей нет.", "en": "No modules loaded."},
            )
        done: list[str] = []
        failed: list[str] = []
        for name in names:
            ...
            except Exception as exc:  # noqa: BLE001 — один сломанный не отменяет остальных
                logger.warning("Модуль %s не перезагрузился: %s", name, exc)
                failed.append(name)
            else:
                done.append(name)
        # Сбойные называем вслух поимённо: «перезагрузил не всё» без имён — это
        # повод идти читать лог, а имя чинит дело сразу.
        said = f"Перезагрузил {len(done)} {plural_form(len(done), _MODULES)}"
        if failed:
            said += f". Не вышло: {', '.join(failed)}"
        return ToolResult.success(
            {"reloaded": done, "failed": failed},
            speech={
                "ru": f"{said}.",
                "en": f"Reloaded {len(done)} modules." + (f" Failed: {', '.join(failed)}." if failed else ""),
            },
        )
