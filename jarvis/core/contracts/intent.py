...
class Utterance:
    ...
    source: str = "voice"
    #: Позвали ли по имени. ``False`` — имени в расшифровке нет, а детектор
    #: услышал его посреди этой же фразы: так выглядят и исковерканные команды,
    #: и слова из песни. Команду такая реплика выполнит, а свободный разговор —
    #: нет (14.09.2026, «Алесса, люблю тебя»).
    named: bool = True
