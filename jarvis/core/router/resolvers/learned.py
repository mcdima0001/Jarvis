...
#: ``core.plan`` и ``core.later`` несут цель **в пересказе модели**: выученной
#: она исполнялась бы дословно при каждом повторе фразы. Живой случай
#: 14.09.2026, 17:14: «не сохраняй» ушло в план, план выключил автопамять, и
...
def generalize(utterance: str, arguments: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    for name, value in arguments.items():
        ...
        if (
            ...
            # Подстановка не может стоять первой. Такой шаблон опознаётся
            # только хвостом, а голову забирает целиком — вместе с чужой
            # командой и её глаголом. Живой случай 24.09.2026: выученное
            # «{question} на экране» поймало «дважды нажми на зелёную кнопку
            # на экране», и вместо нажатия ассистент спросил у зрячей модели
            # «дважды нажми на зелёную кнопку», а та ответила «Готово». То
            # есть мышь не шевельнулась, а вслух прозвучал успех. Первое
            # слово — единственная опора, по которой видно, о чём вообще
            # речь; то же правило и по той же причине держит резолвер `loose`.
...
class LearnedResolver:
    ...
    def _match(
        ...
        for key in known:
            # Шаблоны, начинающиеся с подстановки, не разбираем вовсе — даже
            # если они уже лежат в памяти с прежних времён. Заводить их мы
            # перестали (см. `generalize`), но выученное живёт месяцами, и
            # ждать, пока оно вытеснится, значит месяцами ловить чужие команды
            # хвостом. Правило то же, что при записи: нет первого слова — нет
            # опоры.
            if key.startswith("{"):
                continue
