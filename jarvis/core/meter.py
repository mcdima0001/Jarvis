...
class Meter:
    ...
    def cpu_stage(self, name: str) -> Iterator[None]:
        """Засечь долгий счёт, который сам раскладывается на несколько потоков.

        Для местного синтеза (Kokoro, Piper): модель считает сразу на нескольких
        ядрах, и настоящие часы занизили бы его во столько раз, сколько ядер
        заняла модель. Живой запуск 14.09.2026: английская реплика Kokoro дала
        «всего 126.7% ядра, прочее 124.8%» — синтез звеном не размечался вовсе.

        Считается процессорное время **всего процесса** за время работы. Шаг
        этих часов на Windows 15.625 мс, но синтез идёт секунды, и шаг тонет.
        Цена — чужое в тот же промежуток (имя и речь, проценты ядра) засчитается
        сюда же; для звена, которое съедает ядро целиком, это погрешность.
        Ожидание сюда не попадает: ждущий поток процессор не тратит.
        """
        if not self._enabled:
            yield
            return
