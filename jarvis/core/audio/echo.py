...
#: Насколько два подряд идущих замера могут разойтись, чтобы считаться одним и
#: тем же. Полсотни миллисекунд — обычный разброс GCC-PHAT на музыке; сотни,
#: как на живом запуске 01.08.2026, означают, что измерять нечего.
_AGREE_MS = 50.0
...
class ReferenceTrack:
    ...
    @property
    def pushed(self) -> int:
        """Сколько сэмплов опоры пришло всего.

        По нему считается **настоящая частота** опорного потока. Она обязана
        совпадать с микрофонной, иначе фильтр гонится за трактом, который
        уезжает от него равномерно, — и никакая подстройка выравнивания этого
        не догонит. Проверить это иначе нельзя: `soundcard` берёт частоту, о
        которой его попросили, и о расхождении не сообщает.
        """
        return self._pushed
...
class EchoCancellingSource:
    ...
    def __init__(
        ...
        *,
        ...
        self._settled = False
        #: Замер, который ждёт подтверждения вторым таким же. Двигать потоки по
        #: одному измерению нельзя: оно плавает на сотни миллисекунд, а каждая
        #: правка сбрасывает подобранный тракт.
        self._measured: int | None = None
        #: Насколько два замера могут разойтись и всё ещё считаться одним.
        self._agree = int(sample_rate * _AGREE_MS / 1000)
        self._rescales = 0
    ...
    def _realign(self) -> None:
        ...
        # Один замер ничего не решает. Живой запуск 01.08.2026: подряд шли
        # −347, +237, +225, +4, +395, −462 мс при уверенности 0.51–0.59 — то
        # есть шум, а не измерение. Каждая правка сбрасывала подобранный тракт,
        # фильтр начинал с нуля, не успевал сойтись и снова «разъезжался»;
        # задержка микрофона доползла до предела в 900 мс, а за сеанс вышло
        # −29 дБ. Поэтому двигаем только то, что подтвердилось дважды подряд.
        previous, self._measured = self._measured, shift
        if previous is None or abs(shift - previous) > self._agree:
            logger.debug(
                "Замер %d мс (уверенность %.2f) пока не подтверждён — жду второй",
                shift * 1000 // self._rate,
                sure,
            )
            return
        self._measured = None
        self._note_drift(shift, now)
    ...
    def _note_drift(self, shift: int, now: float) -> None:
        ...
        Микрофон и петлевой захват — **разные устройства с разными кварцами**, и
        часы у них идут чуть по-разному. Для фильтра это хуже, чем разовая
        ошибка выравнивания: тракт не стоит на месте, а медленно едет, и
        подобранные коэффициенты устаревают быстрее, чем успевают сойтись.

        Мерить это стало возможно только теперь, когда замер перестал быть
        шумом: в логе 01.08.2026 подряд шли 179, 187, 175, 179, 170, 170, 179,
        185, 169, 176 мс при уверенности около 0.9 — то есть разброс в 18 мс за
        четыре минуты. Отсюда и цифра в ppm: она отвечает, кварцы это или
        случайность, и без неё разговор был бы гаданием.
        """
        if self._drift_at:
            gone = shift - self._drift_from
            passed = now - self._drift_at
            if passed > 1.0:
                logger.info(
                    "Сдвиг потоков %d мс, за %.0f с уполз на %+d мс (%+.0f ppm)",
                    shift * 1000 // self._rate,
                    passed,
                    gone * 1000 // self._rate,
                    gone / self._rate / passed * 1e6,
                )
        self._drift_from = shift
        self._drift_at = now
