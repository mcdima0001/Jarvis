...
#: Соперники имени в словаре детектора. Грамматика «джарвис + всё остальное»
#: оставляла декодеру выбор из одного слова, и любая похожая речь становилась
#: именем: стенд 19.09.2026 (`tools/wake_bench.py`) — 1.4 ложных в минуту на
#: речи самого ассистента. С частыми и созвучными словами чужая речь находит
#: себе слово: ложных 15 → 2 на 24 минутах, 4 → 0 на живых фразах, обращений
#: по-прежнему 20 из 20 (и с шумом). Цена — 3.4% ядра вместо 2.0%.
RIVALS = (
    "жарко", "жар", "жаркий", "жарить", "джаз", "джинсы", "дарить", "давай", "даже", "да", "дальше",
    "вас", "вы", "висит", "вес", "весь", "виза", "ворс", "варить", "вариант", "армия", "арбуз",
    "царь", "шар", "чары", "чарли", "харрис", "парис", "борис", "барс", "марс", "карта", "сервис",
    "это", "что", "как", "так", "вот", "ну", "там", "тут", "есть", "был", "было", "нет", "где",
    "сейчас", "ладно", "короче", "типа", "просто", "можно", "нужно", "хорошо", "сэр",
)
    ...
    **Срабатывание идёт по частичному результату, а не по концу фразы.** Ждать
    ...
    def __init__(
        ...
        *,
        ...
        others = [word for word in dict.fromkeys(rivals) if word not in self._words]
        self._grammar = json.dumps([*self._words, *others, UNKNOWN], ensure_ascii=False)
        ...
        self._age_ms = 0.0
        logger.info(
            "Активация по звуку (vosk): %s, словарь: %s%s",
            model.name,
            ", ".join(self._words),
            f" и {len(others)} слов-соперников" if others else "",
        )
