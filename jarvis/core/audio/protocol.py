...
class SelectableSink(Protocol):
    ...
    Отдельным протоколом по той же причине, что и поток: это умение. Вывод,
    открывающий устройство на каждую реплику, меняет его бесплатно; вывод,
    держащий устройство открытым, должен был бы его переоткрывать.
    """

    @property
    def device(self) -> str | int | None:
        """Куда сейчас идёт звук: номер, имя или ``None`` — системный выход."""
        ...

    def select(self, device: str | int | None) -> None:
        """Со следующей реплики звучать через это устройство."""
        ...
