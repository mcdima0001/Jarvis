...
#: Во сколько раз оценка эха может превышать то, что за это время играло.
#: Комната звук не усиливает, так что настоящий множитель меньше единицы; сто —
#: запас на переходные секунды при сходимости. Больше — это уже не эхо, а
#: численный разлёт фильтра.
_ABSURD = 100.0
...
#: Как часто напоминать о разлёте, в блоках. Пять секунд: реже — пропустишь
#: начало, чаще — строка перестаёт что-либо сообщать.
_TELL_EVERY = 320
    ...
    * **Когда говорит человек, обучение притормаживает само** (`_pace`). Это
    ...
    def __init__(
        ...
        *,
        ...
        self._muted = 0
        self._blowups = 0
        #: На каком блоке в последний раз жаловались на разлёт.
        self._told = -_TELL_EVERY
    ...
    def process(self, mic: numpy.ndarray, reference: numpy.ndarray) -> numpy.ndarray:
        ...
        # Предохранитель от бесконечности. Мера тут — **опора, а не микрофон**:
        # комната звук не усиливает, поэтому оценка эха не может быть намного
        # громче того, что за это время играло. Сравнивать с микрофоном
        # оказалось нельзя, и это выяснилось на стенде: когда опора резко
        # стихает, фильтр ещё сотню миллисекунд честно предсказывает хвост
        # прежнего громкого куска, микрофон уже тихий — и проверка срабатывала
        # на ровном месте, обнуляя исправный фильтр.
        #
        # Проверка стоит **после** проверки на выключенный микрофон: там свои
        # причины, и порядок между ними важен.
        supply = float(numpy.sum(numpy.abs(self._history) ** 2)) / self._size
        if float(numpy.dot(echo, echo)) > _ABSURD * supply + _EPS:
            self.forget()
            self._blowups += 1
            self._blocks += 1
            # Предупреждение — раз в несколько секунд, а не на каждый блок.
            # Живой запуск 01.08.2026: 184 одинаковые строки за восемнадцать
            # секунд, и за ними перестало быть видно всё остальное. Строка о
            # поломке, повторённая двести раз, о поломке уже не сообщает.
            if self._blocks - self._told >= _TELL_EVERY:
                logger.warning(
                    "Фильтр разошёлся и обнулён (всего %d раз). Эхоподавление сейчас "
                    "не работает, но и не мешает: пустой фильтр отдаёт микрофон как есть",
                    self._blowups,
                )
                self._told = self._blocks
    ...
    def _adapt(self, spectrum: numpy.ndarray, error: numpy.ndarray) -> None:
        ...
        # Знаменатель шага — это энергия опоры **в самом фильтре**, и считать её
        # надо честно, по истории спектров. Раньше тут стояло «мощность
        # текущего блока, умноженная на число разделов», и это приближение
        # верно, только пока громкость опоры ровная. Стоит ей упасть — выключили
        # звук, пропал буфер, пауза между треками, — как в истории остаются
        # громкие куски, а знаменатель считается по тихому блоку. Шаг тогда
        # вырастает во столько раз, во сколько упала громкость.
        #
        # Отсюда и «убрано −369 дБ» в живом логе 01.08.2026: число не плохое, а
        # бессмысленное — фильтр улетал в бесконечность. Предохранитель ловил
        # разлёт и обнулял фильтр, но следующий же блок повторял всё сначала:
        # 184 обнуления за восемнадцать секунд.
        energy = numpy.sum(numpy.abs(self._history) ** 2, axis=0)
        self._typical = max(float(numpy.mean(energy)), _TYPICAL_DECAY * self._typical)
        # Пол — от привычной энергии, а не абсолютный: на тихой опоре шаг просто
        # перестаёт быть большим, вместо того чтобы делиться на почти ноль.
        self._weights += (self._step * pace) * gradient / (
            energy + max(_EPS, _FLOOR * self._typical)
        )
