...
async function jarvisRunPlan(plan) {
  ...
  //: Сколько ждать, что нажатие на «играть» и правда что-то запустило.
  //: Смена волны на Яндекс Музыке занимает секунды: сайт заново запрашивает
  //: подборку, и mediaSession называет новый трек далеко не сразу.
  ...
  */
  ...
  const playItem = async (step) => {
    ...
    // ссылкой на файл.
    //
    // Сколько ждать — вопрос не вкуса. Полторы секунды хватало треку в списке,
    // но не хватает **смене волны**: на Яндекс Музыке «Блюз-рок» — это плитка
    // настройки «Моей волны», и по нажатию сайт заново запрашивает подборку.
    // Живой случай 01.08.2026: нажатие прошло, а через 1.2 с mediaSession всё
    // ещё называл прежний трек — и Jarvis отчитался «нашёл, но включить не
    // смог». Хуже того, следом он нажимал саму плитку, то есть отменял своё же
    // нажатие. Предел расширения тут 12 с, так что три секунды по карману.
    if (pressed && (await awaitStart(control, PLAY_WAIT, before))) {
